...
数组的第 n 个表示当前处理的单个数据包所切出的第 n 个分片期望的长度，最后一项则持续对当前数据包后续切出的分片生效。除最后一项外可以为 0（否则会导致无限空转），对于 TCP 流切片表现为不发送数据，对于 tlshello 则表现为发送违反 RFC 的空 TLS record（部分实现容忍，但是不包括 golang）。
